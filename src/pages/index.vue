          ...
          <div class="card px-3">
            <div class="card-body">
              ...
              <div class="add-items d-flex">
                <input v-model="title" type="text" class="form-control todo-list-input" placeholder="What do you need to do today?">
                <button @click="addTask" class="add btn btn-primary font-weight-bold todo-list-add-btn">Add</button>
              </div>
              <div class="list-wrapper">
                <ul class="d-flex flex-column-reverse todo-list">
                  <Task v-for="(task, i) of tasks" :task="task" :key="i"/>
                </ul>
              </div>
            </div>
          </div>
...
export default {
  ...
  created() {
    this.$store.dispatch('task/getTasks').then(() => {
